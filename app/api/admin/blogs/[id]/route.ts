...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if ((session as any)?.user?.email !== "admin@lakhotiaeyecentre.com") {
  ...
  try {
    ...
    const updated = await withRlsContext(prisma, session as any, (tx) =>
      tx.blog.update({
        where: { id: params.id },
        data: {
          title: parsed.title,
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    await withRlsContext(prisma, session as any, (tx) => tx.blog.delete({ where: { id: params.id } }));
